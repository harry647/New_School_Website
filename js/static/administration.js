...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        const res = await fetch('/data/static/admin-data.json?t=' + Date.now()); // bust cache
        ...
        if (data.bom?.length && grids.bom) {
            ...
            data.bom.forEach(member => {
                ...
                card.innerHTML = `
                    <img src="${member.photo || '/assets/images/defaults/default-user.png'}"
        ...
        if (data.leadership?.length && grids.leadership) {
            ...
            data.leadership.forEach((person, i) => {
                ...
                card.innerHTML = `
                    <img src="${person.photo || '/assets/images/defaults/default-user.jpg'}"
